...
export class NumberingService {
  ...
  public generateCounterKey(
    ...
    // UMUM, SK, dan PERDIR menggunakan satu sequence bersama.
    const sharedTypes = ['UMUM', 'SK', 'PERDIR'];
    if (sharedTypes.includes(String(typeCode).toUpperCase())) {
      const base = 'GENERAL';
  ...
  public async previewMultipleNumbers(params: {
  format?: string;
  typeCode: string;
  unitCode?: string;
  padding?: number;
  ...
  return Array.from({ length: count }, (_, index) => {
    return formatLetterNumber(safeFormat, {
  ...
  /**
   * Find the immediately preceding CANCELLED reservation for this sequence.
   * UMUM, SK and PERDIR share the same counter group.
   */
  private async findPreviousCancelledNumber(
    typeCode: string,
    unitCode: string | undefined,
    year: number,
    month: number,
    ...
    if (previousSequenceNumber < 1) return undefined;

    const sharedTypes = ['UMUM', 'SK', 'PERDIR'];
    const isShared = sharedTypes.includes(String(typeCode).toUpperCase());
    ...
    if (isAtlasConnected()) {
      ...
      if (isShared) {
        query.typeCode = { $in: sharedTypes };
      } else {
        query.typeCode = typeCode;
        if (unitCode) query.unitCode = unitCode;
      }

      const previous = await NumberReservationModel.findOne(query)
        .sort({ cancelledAt: -1, createdAt: -1 });

      return previous?.number || undefined;
    }

    const candidates = inMemoryDB.numberReservations
      .filter((r: any) => {
        if (
          r.year !== year ||
          r.month !== month ||
          r.sequenceNumber !== previousSequenceNumber ||
          r.status !== NumberStatus.CANCELLED
        ) {
          return false;
        }

        if (isShared) {
          return sharedTypes.includes(String(r.typeCode).toUpperCase());
        }

        return r.typeCode === typeCode &&
          (!unitCode || r.unitCode === unitCode);
      })
      .sort((a: any, b: any) => {
    ...
    return candidates[0]?.number || undefined;
  }
  ...
  public async reserveNumber(params: ReserveParams): Promise<any> {
    ...
    const previousCancelledNumber = await this.findPreviousCancelledNumber(
      typeCode,
      unitCode,
      year,
      month,
      ...
      try {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const reservationData = {
            ...
            previousCancelledNumber: i === 0 ? previousCancelledNumber : undefined,
            createdAt: now.toISOString(),
            year,
            month,
            sequenceNumber
  ...
  public async cancelNumber(
    ...
    if (isAtlasConnected()) {
      ...
      if (reservation) {
        ...
        await this.syncCancelledReservationDocuments(
          reservation.number,
          reservation.typeCode,
          reservation.status,
          reason,
  ...
  /**
   * Keep all management modules consistent with the NumberReservation status.
   */
  private async syncCancelledReservationDocuments(
    number: string,
    typeCode: string,
    status: NumberStatus,
    reason: string,
    ...
    try {
      if (isAtlasConnected()) {
        ...
        const outgoing = inMemoryDB.outgoingLetters.find(
          o => o.number === number || o.letterNumber === number
        );
        updateMemory(outgoing);

        if (typeCode === 'UMUM') {
          updateMemory(
            inMemoryDB.generalLetters.find(g => g.number === number)
          );
        }

        if (typeCode === 'SPO') {
          updateMemory(
            inMemoryDB.sopDocuments.find(s => s.number === number)
          );
        }

        if (typeCode === 'PERDIR') {
          updateMemory(
            inMemoryDB.perdirDocuments.find(p => p.number === number)
          );
        }

        if (typeCode === 'SK') {
          updateMemory(
            inMemoryDB.skDocuments.find(k => k.number === number)
          );
        }
      }
    } catch (syncError) {
      console.warn(
        `⚠️ Gagal sinkron status pembatalan ${number}:`,
        syncError
      );
    }
  }
