...
import { IDocumentSk } from '../types';
import { Award, Plus, RefreshCw, Eye } from 'lucide-react';

export const DokumenSkPage: React.FC = () => {
  const [docs, setDocs] = useState<IDocumentSk[]>([]);
  const [loading, setLoading] = useState(true);
  const [numberingHistory, setNumberingHistory] = useState<any[]>([]);
  const [showModal, setShowModal] = useState(false);
  const [selectedDoc, setSelectedDoc] = useState<IDocumentSk | null>(null);

  const [title, setTitle] = useState('');
  const [about, setAbout] = useState('');
  const [considering, setConsidering] = useState('');
  const [inViewOf, setInViewOf] = useState('');
  const [decides, setDecides] = useState('');
  ...
  const handleCreate = async () => {
    if (!title || !about || !decides) {
      alert('Mohon lengkapi Judul, Tentang, dan Mengutus/Menetapkan.');
      return;
    }
    try {
      await apiService.createSkDocument({
        title,
        about,
        considering: considering || 'Bahwa untuk kelancaran pelayanan...',
        inViewOf: inViewOf || 'UU No. 44 Tahun 2009',
        decides
      });
      setShowModal(false);
      setTitle('');
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Dokumen SK (Surat Keputusan Direktur)</h2>
          <p className="text-xs text-slate-500">Penomoran otomatis format: &#123;NO&#125;/SK/DIR/RSSBK/&#123;BULAN_ROMAWI&#125;/&#123;TAHUN&#125;</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="text-xs font-medium">Memuat dokumen SK...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs text-slate-600">
              <thead className="bg-slate-50 text-slate-500 uppercase font-bold text-[11px] border-b border-slate-200">
                <tr>
                  ...
                  <th className="p-3.5">Judul SK</th>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full space-y-4 shadow-2xl">
            <h3 className="font-bold text-slate-900 text-base">Buat SK Baru</h3>
            <div className="space-y-3 text-xs">
              <div>
                <label className="font-bold text-slate-700">Judul SK *</label>
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Surat Keputusan Direktur..." className="w-full p-2.5 border rounded-xl" />
              </div>
              <div>
                <label className="font-bold text-slate-700">Tentang *</label>
                <input type="text" value={about} onChange={(e) => setAbout(e.target.value)} placeholder="Tentang Penetapan..." className="w-full p-2.5 border rounded-xl" />
              </div>
              <div>
                <label className="font-bold text-slate-700">Menetapkan / Memutuskan *</label>
                <textarea value={decides} onChange={(e) => setDecides(e.target.value)} placeholder="Poin-poin keputusan direktur" className="w-full p-2.5 border rounded-xl" rows={3} />
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={handleCreate} className="w-1/2 py-2.5 bg-emerald-600 text-white font-bold rounded-xl text-xs shadow-md">Terbitkan SK</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b pb-2">
              <h3 className="font-bold text-slate-900 text-base">Detail SK Direktur</h3>
            ...
            <div className="space-y-2 text-xs text-slate-800">
              ...
              <div><strong>Diktum Keputusan:</strong> <p className="bg-slate-50 p-2.5 rounded-xl mt-1 border whitespace-pre-line">{selectedDoc.decides}</p></div>
            </div>
